%% Data with normally distributed residuals
...
bState = [0 residSig*0.5 residSig*1.5];  % bonus by each state, state B weak, state C stronger
...
tbRes = table;
tbRes.predictor = ["State B", "State C", "IQ", "Height"]';
tbRes.true = [bState(2:end)'; bIq; bHeight];
...
tbResLM = tbRes
% all CIs include the true values


%% Check if BootLM works similarly
...
tbResBoot_residGrp
tbResBoot_obsGrp
% when testing with rng(shuffle), this approach produced the best result among BootLM
% options. 
